...
const ImagePlaceHolderComponent = ({ onPress, plus = false, size = Matrics.ms241, borderRadius = Matrics.ms120, padding = Matrics.hs50, marginVertical = Matrics.vs40, setImage, image, borderColor = Colors.DARKGRAY, multiple, backgroundColor = "none", disabled }) => {
    const [visible, setVisible] = useState(false)
    const [imageUrl, setImageUrl] = useState()
    return (
        <View style={[styles.mainViw, { marginVertical }]}>
            <Pressable onPress={() => onPress ? onPress() : setVisible(true)} disabled>
                <View style={[styles.imageView, { width: size, height: size, borderRadius, borderColor, backgroundColor }]} >
                    ...
                    {plus ? <Pressable hitSlop={30} onPress={() => onPress ? onPress() : setVisible(true)} style={{ position: "absolute", width: size, height: size, alignItems: "center", justifyContent: "center" }}>
                        <Image source={Images.close} style={{ width: size - 20, height: size - 20, tintColor: Colors.LIGHTBLACK, transform: [{ rotate: "45deg" }], tintColor: Colors.BLUE, resizeMode: "contain" }} />
