                                    ...
                                    statusArray?.map((item, index) => {
                                        return (
                                            <Pressable onPress={() => {
                                                ...
                                                setStatus(item?.value)}} style={{ minWidth: Matrics.hs105, paddingHorizontal: Matrics.hs16, paddingVertical: Matrics.vs12, backgroundColor: status === item?.value ? Colors.MEDIUMREDOPACITY : Colors.BACKGROUNDGRAY, marginRight: Matrics.vs10, marginVertical: Matrics.vs7, borderRadius: Matrics.ms25, justifyContent: "center" }}>
                                                    ...
                                                    { item?.requirement?._id &&  (status === "Upcoming"||status === "Previous") ? item?.requirement?.images?.map(img => {
                                                        return (
                                                            <View style={{ marginRight: Matrics.hs15 }}>
                                                                ...
                                                                <ImagePlaceHolderComponent size={Matrics.ms60} borderRadius={Matrics.ms0} padding={Matrics.hs10} marginVertical={Matrics.vs15} setImage={(image) => setImage(image)} image={`${IMAGE_URL}${img}`} disabled={true} borderColor={Colors.WHITE} />
                                                            </View>
                                                        )
                                                    }):null}
